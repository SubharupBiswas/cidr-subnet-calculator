...
const SectionCard: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/80 rounded-xl p-6 flex flex-col gap-4">
    {children}
  </div>
);

const CodeBadge: React.FC<{ children: React.ReactNode; color?: 'teal' | 'cyan' | 'amber' | 'purple' | 'rose' | 'zinc' }> = ({
  children,
  color = 'teal',
}) => {
  const palette: Record<string, string> = {
    ...
    purple: 'bg-purple-50 text-purple-700 border-purple-100 dark:bg-purple-500/10 dark:text-purple-400 dark:border-purple-500/20',
...
export const SubnetGuide: React.FC = () => {
  // Build prefix reference matrix /8 → /32
  const matrixRows = [];
  for (let p = 8; p <= 32; p++) {
  ...
  const prose = 'text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed';
  const heading = 'text-lg font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2.5';
  const strong  = 'font-semibold text-zinc-800 dark:text-zinc-200';

  return (
    <div className="bg-white border border-zinc-200/80 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] dark:bg-[#0d0e15]/80 dark:border-white/[0.04] dark:shadow-[inset_0_1px_2px_rgba(255,255,255,0.01)] rounded-2xl p-6 md:p-8 transition-all duration-300 w-full flex flex-col gap-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        {/* ── Left column: four documentation sections ── */}
        <div className="lg:col-span-7 flex flex-col gap-8">
          ...
          <SectionCard>
            ...
            <div className="bg-zinc-100 dark:bg-[#090a0d] border border-zinc-200 dark:border-zinc-800 rounded-lg p-4 font-mono text-sm flex flex-col gap-2">
              ...
              <span className="text-zinc-700 dark:text-zinc-200">10.0.0.0<span className="text-cyan-600 dark:text-cyan-400">/24</span>
                <span className="ml-4 text-zinc-400 dark:text-zinc-500"># 254 usable hosts — Class C equivalent</span>
              </span>
              <span className="text-zinc-700 dark:text-zinc-200">172.16.0.0<span className="text-cyan-600 dark:text-cyan-400">/20</span>
                <span className="ml-4 text-zinc-400 dark:text-zinc-500"># 4,094 usable hosts — mid-size segment</span>
              </span>
              <span className="text-zinc-700 dark:text-zinc-200">192.168.1.128<span className="text-cyan-600 dark:text-cyan-400">/26</span>
                <span className="ml-4 text-zinc-400 dark:text-zinc-500"># 62 usable hosts — fine-grained split</span>
              </span>
            </div>
            <p className={prose}>
          ...
          <SectionCard>
            ...
            <div className="flex flex-col gap-4">
              ...
              {/* Broadcast Address */}
              <div className="flex gap-3">
                <span className="mt-0.5 w-6 h-6 shrink-0 rounded-md bg-amber-100 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 flex items-center justify-center text-[10px] font-bold text-amber-700 dark:text-amber-400">2</span>
                <div className="flex flex-col gap-1">
              ...
              {/* Host Capacity */}
              <div className="flex gap-3">
                <span className="mt-0.5 w-6 h-6 shrink-0 rounded-md bg-emerald-100 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 flex items-center justify-center text-[10px] font-bold text-emerald-700 dark:text-emerald-400">3</span>
                <div className="flex flex-col gap-1">
          ...
          <SectionCard>
            ...
            <div className="bg-purple-50 dark:bg-purple-500/5 border border-purple-200 dark:border-purple-500/20 rounded-xl p-5 flex flex-col gap-4">
              <p className="text-sm font-semibold text-purple-800 dark:text-purple-300">
                Goal: Split <CodeBadge color="purple">172.16.0.0/16</CodeBadge> into at least <strong>10 equal subnets</strong>.
              </p>
              <ol className="list-none flex flex-col gap-3 text-sm text-zinc-600 dark:text-zinc-400">
        ...
        {/* ── Right column: Prefix Reference Matrix ── */}
        <div className="lg:col-span-5">
          <div className="border border-zinc-200 dark:border-zinc-800/80 rounded-xl bg-zinc-50 dark:bg-zinc-950/40 overflow-hidden flex flex-col sticky top-8">
            <div className="bg-zinc-100 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 px-4 py-3 flex items-center justify-between">
              <h4 className="font-bold text-sm text-zinc-700 dark:text-zinc-300 tracking-tight">
                Prefix Scaling Matrix
              </h4>
              <span className="text-[10px] font-mono text-zinc-400 dark:text-zinc-500">/8 → /32</span>
            </div>
            <div className="overflow-y-auto custom-scrollbar" style={{ maxHeight: '680px' }}>
              <table className="w-full text-left border-collapse font-mono text-xs">
                <thead className="sticky top-0 bg-zinc-50/90 dark:bg-zinc-950/90 backdrop-blur-sm z-10 border-b border-zinc-200 dark:border-zinc-800">
