...
export function FaqAccordion() {
  ...
  const faqs: FAQItem[] = [
    ...
    },
    {
      ...
      answer: (
        <div className="text-zinc-700 dark:text-zinc-400 text-sm leading-relaxed space-y-2">
          <p>
            To calculate the total number of usable hosts in an IPv4 subnet, use the formula:
          </p>
          <div className="bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/50 rounded-lg p-3 my-2 font-mono text-center text-teal-800 dark:text-teal-400 text-base">
          ...
          <p className="text-xs text-zinc-700 italic">
            *Note: Under RFC 3021, a <code className="text-zinc-700 dark:text-zinc-400 font-mono">/31</code> subnet has 2 usable hosts (used for point-to-point links), and a <code className="text-zinc-700 dark:text-zinc-400 font-mono">/32</code> subnet represents a single host (1 usable host) with no network or broadcast address overhead.
          </p>
        </div>
      )
    },
    {
      ...
      answer: (
        <ul className="text-zinc-700 dark:text-zinc-400 text-sm leading-relaxed list-disc pl-5 space-y-2">
          <li>
    ...
    },
    {
      ...
      answer: (
        <p className="text-zinc-700 dark:text-zinc-400 text-sm leading-relaxed">
          A subnet mask is a 32-bit mask used to divide an IP address into subnets and specify the network's available hosts. The CIDR prefix indicates the count of contiguous leading "1" bits in the subnet mask. For example, a CIDR prefix of <code className="text-teal-800 dark:text-teal-400 font-mono">/24</code> translates to twenty-four "1" bits and eight "0" bits (<code className="text-zinc-700 dark:text-zinc-500 font-mono">11111111.11111111.11111111.00000000</code>), which in dotted-decimal notation is written as <code className="text-teal-800 dark:text-teal-400 font-mono">255.255.255.0</code>.
        </p>
      )
    }
  ];
  ...
  return (
    <section aria-labelledby="faq-title" className="w-full">
      <div className="flex items-center gap-3 pb-4 mb-2">
        <HelpCircle className="w-5 h-5 text-teal-800 dark:text-teal-400" />
          ...
          return (
            ...
            >
              ...
              >
                ...
                <ChevronDown 
                  className={`w-4 h-4 text-zinc-700 dark:text-zinc-500 shrink-0 transition-transform duration-300 transform ${
