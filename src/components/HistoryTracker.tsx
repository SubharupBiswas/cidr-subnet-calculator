    ...
    <div className="bento-card bento-card-hover p-5 flex flex-col gap-5">
      <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 pb-4">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 tracking-tight font-mono uppercase tracking-widest">Recent Calculations</h2>
        </div>
        {history.length > 0 && (
          <button
      ...
      {history.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-8 text-center">
          <History className="w-7 h-7 text-zinc-400 dark:text-zinc-700 mb-2" />
          <p className="text-xs text-zinc-500 dark:text-zinc-400 font-mono">No history yet</p>
        ...
        <div className="flex flex-col gap-1.5 max-h-[280px] overflow-y-auto scrollbar-none">
          {history.map((item) => (
            <div
              key={item.id}
              className="flex items-center justify-between py-2 px-2 rounded-lg border border-zinc-200 dark:border-zinc-800/60 hover:bg-zinc-200/40 dark:hover:bg-zinc-800/40 hover:border-zinc-300 dark:hover:border-zinc-700/60 transition-all group"
            >
              <button
              ...
              >
                <span className="font-mono text-sm font-bold text-zinc-700 dark:text-zinc-300 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors flex items-center gap-2">
                  <span className="text-zinc-400 dark:text-zinc-700 font-normal select-none text-xs">$_</span>
                  {item.ip}/{item.prefix}
                </span>
