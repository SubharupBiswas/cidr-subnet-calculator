    ...
    <div className={containerClasses}>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 tracking-tight font-mono uppercase tracking-widest">Subnet Splitter</h2>
        </div>
        
        {/* Split Target Controls */}
        <div className="flex items-center gap-3 bg-zinc-100 dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 rounded-xl px-3 py-1.5 self-start md:self-auto hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors">
          <span className="text-[10px] font-semibold tracking-wider text-zinc-500 dark:text-zinc-400 uppercase">Split into</span>
          <select
            value={targetPrefix}
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="overflow-x-auto w-full max-w-full border border-zinc-250 dark:border-zinc-800/60 rounded-xl bg-zinc-100/30 dark:bg-zinc-950/30 scrollbar-none">
          <table className="w-full text-left border-collapse font-mono text-xs">
            <thead>
              <tr className="bg-zinc-200/50 dark:bg-zinc-900/60 border-b border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 font-semibold tracking-wider text-[9px] uppercase font-mono">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/40">
              {subnetsList.map((sub, idx) => (
                <tr key={idx} className="hover:bg-zinc-200/30 dark:hover:bg-zinc-800/30 transition-colors text-zinc-700 dark:text-zinc-300 group">
                  ...
                  <td className="py-3 px-4 text-right text-zinc-700 dark:text-zinc-400 font-bold">{sub.hosts.toLocaleString()}</td>
                  <td className="py-3 px-4 text-center">
                    <button
                      onClick={() => onLoadSubnet(sub.networkIp, targetPrefix)}
                      className="p-1.5 text-zinc-400 dark:text-zinc-500 hover:text-cyan-600 dark:hover:text-cyan-400 hover:bg-cyan-500/10 rounded-md transition-colors inline-flex items-center justify-center cursor-pointer"
                      title="Load Subnet"
                    >
        ...
        {isCapped && (
          <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-450 bg-zinc-100/60 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3">
